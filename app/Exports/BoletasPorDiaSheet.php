...
class BoletasPorDiaSheet implements FromQuery,WithTitle,WithHeadings,ShouldAutoSize,WithEvents,WithColumnFormatting
{
    /**
    ...
    */
    ...
    public function query()
    {
        $dia_bol = pago::query()->selectRaw('CONVERT(pagos.created_at,TIME) as hora, "boleta" as tipo_doc, boletas.num_boleta as num_doc, pagos.monto as total, pagos.referencia as referencia')
            ...
            ->whereRaw('pagos.tipo_doc=? AND pagos.fecha_pago=? AND pagos.activo=? AND (pagos.id_forma_pago=? OR pagos.id_forma_pago=?) AND pagos.referencia_pago=?', ['bo', $this->fecha, 1, 2, 5,1])
    ...
    {
        return 'Boletas Transbank ' . $this->fecha;
    }
