...
class FacturasPorDiaSheetGetnet implements FromQuery,WithTitle,WithHeadings,ShouldAutoSize,WithEvents,WithColumnFormatting
{
    /**
    ...
    */
    ...
    public function query()
    {
        $dia_fac = pago::query()->selectRaw('CONVERT(pagos.created_at,TIME)  as hora,"factura" as tipo_doc, facturas.num_factura as num_doc, pagos.monto as total, pagos.referencia as referencia')
            ...
            ->whereRaw('pagos.tipo_doc=? AND pagos.fecha_pago=? AND pagos.activo=? AND (pagos.id_forma_pago=? OR pagos.id_forma_pago=?) AND pagos.referencia_pago=?', ['fa', $this->fecha, 1, 2, 5,2])
    ...
    {
        return 'Facturas Getnet ' . $this->fecha;
    }
