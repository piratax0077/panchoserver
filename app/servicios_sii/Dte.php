...
class Dte
{
    ...
    private function loadXML($xml)
    {
        if (!empty($xml)) {
            ...
            if (isset($this->getDatos()['@attributes'])) {
                ...
                $this->id = 'PR_T'.$this->tipo.'F'.$this->folio;
            }
            return true;
        }
        return false;
    ...
    public function timbrar(Folios $Folios)
    {
        ...
        //Si es boleta, no viene Receptor
        if(!$this->xml->xpath('/DTE/'.$this->tipo_general.'/Encabezado/Receptor/RUTRecep')->item(0)){
            $RR='60803000-K'; //RUT DE SII
        }else{
        ...
        if (openssl_sign($DD, $timbre, $Folios->getPrivateKey(), OPENSSL_ALGO_SHA1)==false) {
            ...
            //echo "error claves openssl_sign";
            return false;
        }
    ...
    public function getResumen()
    {
        ...
        if ($this->esBoleta()) {
            ...
            if (!$resumen['MntNeto']) {
                list($resumen['MntNeto'], $resumen['MntIVA']) = $this->calcularNetoIVA($resumen['MntTotal']-$resumen['MntExe'], $resumen['TasaImp']); //ORIGINAL
                //list($resumen['MntNeto'], $resumen['MntIVA']) = $this->calcularNetoIVA($resumen['MntTotal']+$resumen['MntExe'], $resumen['TasaImp']); //yo lo puse con signo +
            }else{
    ...
    {
        // comenté xq la boleta necesita la tasa
    ...
    private function normalizar(array &$datos)
    {
        ...
        if (!empty($datos['Encabezado']['Receptor']['RUTRecep'])) {
...
/*
        if($this->esBoleta()){
            $datos['Encabezado']['Totales']['MntTotal']=0; //Porque abajo lo recalcula...
        }
*/
        foreach ($datos['Detalle'] as &$d) {
            ...
            if ($d['MontoItem']) {
                // si no es boleta
                if (!$this->esBoleta()) { //original if (!$this->esBoleta()) {
                    ...
                    if (!empty($d['IndExe'])) { // si es exento
                        if ($d['IndExe']==1) {
                            $datos['Encabezado']['Totales']['MntExe'] += $d['MontoItem'];
                        }
                    }
                    // agregar al monto total
                    $datos['Encabezado']['Totales']['MntTotal'] += $d['MontoItem']; //original
                }
            }
        }
    ...
    private function normalizar_boletas(array &$datos)
    {
        ...
        $datos['Encabezado']['Transporte'] = false; //https://github.com/LibreDTE/libredte-lib/commit/a66de48804421b27ef6439cb4b3ef3bd9f1c5a35#diff-0ed44bbc5a85f239319a7014472acc75R1734
    ...
    public function getUltimaAccionRCV(FirmaElectronica $Firma)
    {
        ...
        $RCV = new RegistroCompraVenta($Firma);
        try {
