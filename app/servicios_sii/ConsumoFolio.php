...
class ConsumoFolio extends Libro
{
    ...
    public function setCaratula(array $caratula)
    {
        ...
        $this->id = 'RCOF_'.str_replace('-', '', $this->caratula['RutEmisor']).'_'.str_replace('-', '', $this->caratula['FchInicio']).'_'.date('U');
    }
    ...
    private function getResumen()
    {
        ...
        foreach ($this->detalles as &$d) {
            ...
            if(isset($d['Anulado']) && $d['Anulado']===true){
                $Resumen[$d['TpoDoc']]['FoliosAnulados']++;
                $RangoAnulados[$d['TpoDoc']][] = $d['NroDoc'];
            }
...
/*
            Debugbar::warning("rangos anulados para ".$r['TipoDocumento']);
            Debugbar::warning($RangoAnulados[$r['TipoDocumento']]);
            Debugbar::warning("rangos utilizados para ".$r['TipoDocumento']);
            Debugbar::warning($RangoUtilizados[$r['TipoDocumento']]);
*/
            ...
            if(count($RangoAnulados[$r['TipoDocumento']])>0)
                $r['RangoAnulados'] = $this->getRangos($RangoAnulados[$r['TipoDocumento']]);
