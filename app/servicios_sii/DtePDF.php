...
class DtePDF extends PDF
{
    ...
    {
        parent::__construct();
        $this->SetTitle('DTE '.config('app.name'));
        $this->setWebVerificacion(Session::get('PARAM_WEB'));
    ...
    {
        //$inicio = date ('Y-m-d h:i:s', date_timestamp_get(new DateTime()));
        ...
        $fin = date ('Y-m-d h:i:s', date_timestamp_get(new DateTime()));
        //Debugbar::info("DtePDF:agregar_papel_80. ".$inicio);
        //Debugbar::info("DtePDF:agregar_papel_80. ".$fin);
    }
    ...
    protected function agregarReceptor(array $Encabezado, $x = 10, $offset = 22)
    {
        ...
        if (!empty($receptor['RznSocRecep']) and $receptor['RUTRecep']!='66666666-6') {
            $this->setFont('', 'B', null);
        ...
        if (!empty($receptor['GiroRecep']) and $receptor['RUTRecep']!='66666666-6') {
            $this->setFont('', 'B', null);
            $this->Texto('Giro', $x);
            $this->Texto(':', $x+$offset);
            $this->setFont('', '', null);
        ...
        if (!empty($receptor['DirRecep']) and $receptor['RUTRecep']!='66666666-6') {
            $this->setFont('', 'B', null);
