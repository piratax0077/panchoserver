  ...
  function borrar_cliente(idc){
    if (confirm('Esta seguro de eliminar el registro '+idc+' ?')==true)
    {
      ...
      $.ajax({
        ...
        success:function(resp){
            Vue.swal.close();
          location.href="{{url('clientes/estado')}}";
...
//Borrar la tabla temporal de descuentos.
  ...
  function clientes_morosos(){
    let url = '/clientes/morosos';
    $.ajax({
      type:'get',
      url: url,
      success: function(clientes_morosos){
        
        let cantidad = clientes_morosos.length;
        let html = '<p>Tiene '+cantidad+' clientes morososo con más de 30 días del último movimiento </p>';
        html += '<ul>';
        clientes_morosos.forEach(cliente => {
          html += '<li>'+cliente.nombres+' '+cliente.apellidos+'</li>';
        });
        html += '</ul>';
        Vue.swal({
          icon:'info',
...
<h4 class="titulazo">Estado de cuenta cliente</h4>
...
<!-- INGRESAR/MODIFICAR CLIENTES -->
    <div class="col-sm-4" style="background-color: #e8ffff; display: none;">
      <div id="mensajes"></div>
      <div class="row">
...
<!-- BUSCAR CLIENTES -->
    <div class="col-sm-12">
      <div class="row" id="panel-busqueda-cliente">
      ...
      <div class="row" id="nombre_cliente"></div>
